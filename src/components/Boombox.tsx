...
export default function Boombox({ frequency, needlePercent, playing, onKnobChange, onPlay, onPrev, onNext, onStop }: BoomboxProps) {
  ...
  // RAF loop — drives waveform only (reels now use CSS animation)
  useEffect(() => {
  ...
  return (
    /* wrapper carries the shadow so the SVG itself has no CSS filter —
       a filter on <svg> makes Safari composite it as a static bitmap,
       which stops all JS-driven SVG attribute updates from repainting */
    ...
    <svg viewBox="0 0 660 430" xmlns="http://www.w3.org/2000/svg"
      ...
      {/* ── Knobs ── */}
      {KNOB_DEFS.map(def => (
        <Knob key={def.key} def={def} value={knobs[def.key]} onChange={v => handleKnob(def.key, v)} />
      ))}
      ...
      <text x="330" y="215" textAnchor="middle" fontFamily="'Courier New', monospace" fontSize="6.5" fill="#4a2808" letterSpacing="0.5" opacity="0.85">for mom ♪♪ --</text>
      ...
      {/* ── Push buttons row (3D raised, interactive) ── */}
